...
const MyOrders = () => {
    ...
    const handleDelete = (a) => {
        fetch(`http://localhost:5000/myOrder/${a}`, {
            method: "DELETE",
            headers: {
    ...
    return (
        <div>
            ...
            <div>
                <div>
                    <Link to='/dashboard/myOrders/hidedOrders' className='btn'> hided Orders </Link>
                </div>
                                    ...
                                    myOrders.map(a =>
                                        <tr >
                                            <th className={`bg-${a.paid ? "info" : ""}`}>
                                                <label>
                                                    <p> {a.quantity} </p>
                                                    {!a.paid && <button onClick={() => handleDelete(a._id)} className='text-rose-500' >Delete  </button>}
                                                    {a.paid && <button onClick={() => handleHide(a._id)} className='text-primary' > hide   </button>}
                                                </label>
                                            </th>
                                                    ...
                                                    <div>
                                                        <div>
                                                            <td className={`bg-${a.paid ? "info" : ""} text-right `} >
                                                                {/* ----------------- */}
                                                                {!a.paid && <button onClick={() => handleDelete(a._id)} className='text-rose-500 btn  ' >Delete  </button>}
                                                                {a.paid && <button onClick={() => handleHide(a._id)} className='text-primary btn  ' > hide   </button>} 
                                                                <p> Total Price: {a.price}</p>
                                                                {/* ------------------ */}
